...
export const Circle: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    //     let check = items[i];
    //     if(selectedFlag.currency_code === check.currency_code){
    ...
    (event: PanGestureHandlerStateChangeEvent) => {
      ...
      switch (event.nativeEvent.state) {
        case State.END: {
          ...
          const index = Math.floor((animateAngle / 380) * items.length);
      ...
      items.map((item, idx) => {
        ...
        return (
          ...
          >
            ...
            color="#333333"
          /> */}
            <Avatar rounded size={45} source={{ uri: item.country_flag }} />
          </View>
        );
      }),
        ...
        >
          ...
          >
            {blurredView ? (
              ...
              <View style={Styles.itemsContainer}>{iconsList}</View>
            )}
          </Animated.View>
